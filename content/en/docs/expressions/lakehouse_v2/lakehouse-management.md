...
weight: 2.0
description: Manage your StarRocks + Iceberg lakehouse from PlaidCloud — run and profile queries, browse the Iceberg catalog, manage warehouses and users, restart compute, and watch cluster load.
...
{{< note >}}
Destructive actions (drop namespace, drop table) are disabled when the selected warehouse is the workspace default.
{{< /note >}}
...
{{< note >}}
A rolling restart briefly interrupts active sessions. Coordinate with users before confirming.
{{< /note >}}
