...
- **Rule-based assignment** — when the spread depends on more than one driver and varies by source, author an allocation dimension with the guided builder instead of wiring a single driver table. See [Creating an Allocation Dimension](/guides/dimensions/dimensions/#creating-an-allocation-dimension)
