...
> **Times are UTC unless you say otherwise.** New events default to the UTC timezone. If "9 a.m." needs to mean 9 a.m. where you are, set the **Timezone** field on the event — otherwise a scheduled run can land hours off from what you expected. Other offsets follow a place's local time, daylight saving included — see [Event Scheduler](/administration/scheduled-events/event-scheduler/#timezone).
