...
> **Times are London time unless you say otherwise.** New events default to **UTC +0**, which is read as London time, British Summer Time included. If "9 a.m." needs to mean 9 a.m. where you are, set the **Timezone** field on the event — otherwise a scheduled run can land hours off from what you expected. Most other offsets follow a place's local time too — see [Event Scheduler](/administration/scheduled-events/event-scheduler/#timezone).
