...
description: End-to-end Sage Intacct tutorial — connect, pull account balances by dimension with the Import Sage step, shape them into a departmental trial balance and P&L, add a per-head metric from statistical accounts, put it on a dashboard, and schedule the refresh.
sidebar:
...
The [Import Sage step](/reference/workflow-steps/sage-intacct/import-sage/) pulls account balances for a date range and an account range, carrying the dimensions you choose onto every row — that's your trial balance.
...
2. Add a [Table Outer Join step](/reference/workflow-steps/tables/table-outer-join/). Set **Table 1** to `intacct_pnl` and **Table 2** to `intacct_headcount`, and join on `departmentname` — now a clean 1:1 match per department. Leave **Join Type** at its default **Left Join** — every P&L row is kept, with headcount filled in where the department matches. Target a new table, `intacct_pnl_with_headcount`, and propagate all columns from both tables.
...
- [Import Sage Intacct Query](/reference/workflow-steps/sage-intacct/import-intacct-query/) — pull any Intacct object (GL accounts, customers, projects) to enrich your statements.
