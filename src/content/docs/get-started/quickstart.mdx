...
If you'd rather see a finished result before you build one, take the shortcut. On a brand-new workspace, a **Load sample** button appears. Click it, and PlaidCloud seeds a ready-made **Retail Sales Analysis** project and runs its workflow, showing you each stage as it goes — **Importing** a retail sales file, **Cleaning** it, then **Summarizing** revenue by region — before opening the result for you. The first run takes about a minute while it warms your warehouse; every run after that is near-instant, and the screen tells you so, so the wait isn't a surprise. You reach a real first result in a few minutes without building anything, and every piece it created is yours to open, change, and rerun. If loading or running the sample hits a problem, PlaidCloud tells you and offers to try again.
