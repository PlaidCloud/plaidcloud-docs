...
| **Default model** | Chosen from the provider's own current line-up, fetched with your API key. **Other…** takes a model id you type, for anything the list does not carry. |
| **Base URL** | Shown only for providers that read one: required for Azure OpenAI (your resource endpoint), and an optional override for OpenAI, Grok and GitHub Models. Anthropic and Gemini do not use it. |
...
Name, memo, and who can see the connection are set here too, and **Test Connection** checks the key
...
A workspace may also hold an older **Anthropic Claude** connection, from before the provider became
a field on one connection type. It holds the same settings and opens in the same editor, with the
provider fixed to Anthropic. Create new connections as **LLM Provider**.

<Aside type="tip">
**The model list comes from the provider, not from PlaidCloud.** Opening the connection asks your
provider which models your key can reach, so a model released this morning is offered without
waiting for a PlaidCloud release. Models that cannot answer a prompt — embeddings, speech, image
and moderation models — are left out.

If the provider cannot be reached (a key that has not been entered yet, a wrong one, or an outage)
the list falls back to PlaidCloud's built-in one and says so. **Other…** still accepts any model id
you type.
</Aside>
