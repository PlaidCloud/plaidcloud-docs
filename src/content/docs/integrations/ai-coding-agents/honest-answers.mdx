...
| --- | --- |
| "This describes the whole pool" | The figure is a total; to see how it shifts *between* members (regions, products, cost centers), ask about a specific one. Where the note names an earlier step in the chain — it opens "Stage 2 (…)" — narrowing your question won't reach that step, so trace its own table instead. |
...
**A note that describes only part of the result says which part.** Some results are built by combining several allocation branches, and a note can be raised by one branch rather than by the whole table — a note quoting figures that belong to a single branch now opens by naming it, as *"Branch 4 (admin costs): …"*. Read an unprefixed note as being about the result you asked for, and a prefixed one as being about that branch alone.
