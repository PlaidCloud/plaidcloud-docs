...
Where a table recombines several allocations and the branches disagree about the column, the answer does not name one branch's table as the one that lacks it. It says instead that not every branch's table has a column of that name — and where *none* of them has one it says that, rather than leaving you to read "not every" as meaning some branch was narrowed after all.
