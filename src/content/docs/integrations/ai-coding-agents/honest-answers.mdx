...
A column you name but leave without any values is treated the same way and said differently, because the cause is different: the column is on the table, and it is the values that are missing. There the answer says the filter gave no values for it — which, unlike a missing column, you can fix by asking again.
...
Where a table recombines several allocations, the branches are separate tables and can disagree about the column — one may carry it while another does not. The answer never names one branch's table as the one that lacks it, and it distinguishes two cases, because they are different answers.

Where some branches carry the column, those branches really were narrowed to the value you asked for, and only the rest cover every value of it:
...
Saying the figures covered all region values would be true of one branch and false of the other, and a reader told that could reasonably discard a figure that is partly what they asked for.

Where *no* table carries the column, the figures really do cover every value of it, and the answer says so — and says that no table has it, rather than the weaker "not every table". Where the branches each carry a different one of the columns you asked for, no single narrowing spans the answer, and it says that neither column is on every table it covers.
