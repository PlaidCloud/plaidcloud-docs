...
## It Names What You Could Narrow By Instead
...
The faithfulness check grew a matching rule. Because a what-if answer deliberately leaves some results without a figure, the risk isn't only an invented number — it's a *real* number from one result printed against another that was never measured, which reads as certainty exactly where the analysis had none. The check now catches that: put a figure beside a result the analysis could not measure, without saying that's what it is, and the rewrite is rejected.
