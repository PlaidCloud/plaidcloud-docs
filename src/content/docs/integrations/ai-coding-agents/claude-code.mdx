...
2. Restart Claude Code. The first time you ask it to use a `plaidcloud_` tool, the bridge will pop up an authorization URL. Open it, sign in to PlaidCloud, and you'll land on an **Application Access Request** page naming the requesting application and the redirect URI it will send credentials to — check that these match Claude Code before approving. Approve it, then paste the callback URL Claude Code asked for. The token is cached locally and refreshed automatically; you won't see this page again until the refresh token expires or is revoked.

3. Verify with `claude mcp list` (CLI) or `/mcp` (in-session). The server should show as connected.

<Aside>The OAuth bridge state can occasionally get stuck on remote/SSH sessions where Claude Code can't reach `localhost` from your browser. If that happens, fall back to Option B.</Aside>
