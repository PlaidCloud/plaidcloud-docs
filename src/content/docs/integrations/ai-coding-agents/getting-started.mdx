...
<Aside type="note">
How far below a folder the search reaches depends on the document account's storage type: an object-store account (S3, Wasabi, GCS) lists the whole tree, while OneDrive, SFTP and Azure Blob accounts list one directory level. If a bare file name is not found on one of those, give the full path.
</Aside>
