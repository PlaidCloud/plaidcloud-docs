...
| --- | --- |
| Import a file into a table | The file, and optionally the project and a name for the new table |
...
Show the candidates to the person you are helping, then call the playbook again with their answer. When the file cannot be found, the reply lists the nearest names instead and reports how many files it searched, so "not there" can be told apart from "not where I looked".

<Aside type="note">
A bare file name is searched for below the folder on every account type. On an object-store account (S3, Wasabi, GCS) that costs one listing; on OneDrive, SFTP and Azure Blob, where the first listing only covers one directory level, the search makes a second, deeper pass — so give the full path when you have it, and expect a bare name to take a little longer on a large account.
</Aside>
