...
The quickest way to connect is inside PlaidCloud itself: click **Connect Your AI** in the header toolbar, just left of the light/dark theme switch, pick the client you use, and follow the steps it shows — including your workspace's MCP server URL, already filled in. It's available whenever your workspace has the AI entitlement enabled. The rest of this page, and the per-client pages that follow it, remain the reference — read on if you're not in the web UI, or want the full picture of authentication and what the server exposes.
...
1. **OAuth 2.1 + PKCE via Dynamic Client Registration (DCR).** This is what Claude.ai's custom-connector UI uses, and it's also the default for Claude Code's MCP bridge. The client registers itself and redirects you to PlaidCloud's Keycloak login. After you sign in, PlaidCloud shows an **Application Access Request** page naming the application and the redirect URI it will send credentials to — approve it once per authorization to get your token. This is a security control, not a formality: check that the application name and redirect URI match the client you're actually connecting, and don't approve a request you didn't start.
...
- [Grok](../grok/) — connecting Grok's custom connectors.
