...
- **Rows.** A find returns 25 matches per page. When a page is capped, the envelope sets `next_cursor` **and** names the clipped list in `truncated` — that combination means "there is more", not "that is all of them". Raise `limit` or page with the cursor.
- **Columns.** Pass `fields=['id', 'name', 'update_time']` to get only those columns back, or `field_set='full'` for the whole record. A field name that appears on no record is an error listing the available fields, not a silent omission — so a typo fails loudly instead of quietly returning less than you asked for.
