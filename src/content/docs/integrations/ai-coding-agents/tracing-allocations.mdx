...
- **Caveats in plain words** — for example: the cause is a mix of overlapping factors rather than a single one; the answer rests on the *current* hierarchy or driver weights; part of a change may be a data-completeness artefact (a period that hasn't finished loading) rather than a real movement; or a what-if figure can't be added across targets.
