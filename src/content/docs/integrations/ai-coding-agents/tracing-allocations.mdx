...
<Aside type="note" title="Every answer carries a confidence signal">
The agent labels each explanation with **how confident** it is and flags when a confident-sounding number would mislead — a year-over-year comparison with no prior-year data or a still-incomplete current period, a whole-group total hiding a member-level reshuffle, factors that move together, or attributed pieces that don't add back to the reported total. The warning rides in the plain-language answer, so you see it before acting on the figure.
</Aside>
