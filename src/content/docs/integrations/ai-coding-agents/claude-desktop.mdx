...
<Aside>Custom Connectors are available on Claude Pro, Max, Team, and Enterprise plans. Team and Enterprise admins can pin connectors for everyone in the workspace.</Aside>
...
5. Sign in, then approve the **Application Access Request** page that follows — it names the requesting application and the redirect URI it will send credentials to, so check both match Claude before approving. You'll be redirected back to Claude with the connection saved.
