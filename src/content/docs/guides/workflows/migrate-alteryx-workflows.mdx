...
9. If the workflow has a step that did not convert, a file it reads is missing, or another issue needs a look, the window stays open instead and shows the conversion report so you can review it before continuing.
...
Importing no longer fails just because a file the workflow reads is missing — a network-share path that was never uploaded, for example. The workflow still converts, and the conversion report lists each missing file with:

- The Document path to upload it to.
- The original Alteryx path it was read from.
- Any sheets the workflow expects from it.
- The Alteryx tools that read it.

For most files, upload it to the path the report names and rerun the workflow — no re-import needed. A few kinds of missing file are read at conversion time rather than at run time, so the report tells you to import the workflow again once the file is in place: an Alteryx flat ASCII layout file, a wildcard input, a Dynamic Input template, and a FirstRow rename source all fall into this group.

<Aside type="caution" title="A file still missing when the workflow runs">
If a converted Input step's only source file is still absent when you run the workflow, the step fails with "Input file not found at &lt;path&gt;" instead of silently reading nothing and reporting success. This also applies to workflows you converted before this change — the fix takes effect the next time they run.
</Aside>

Other problems in a workflow — a macro or flat-file layout PlaidCloud can't read, a file that already exists at the target path with different content, a tool that can't be converted — flag that one step with a warning instead of failing the whole import, so the rest of the workflow still comes across.

### When The Import Is Refused

If the file you upload can't be converted at all, the import is refused outright rather than producing a partial result, and the error tells you what to do. This happens when the file isn't an Alteryx workflow; when it is an Alteryx package (`.yxzp`), in which case export its `.yxmd` workflow from Alteryx Designer and upload that with the files it uses; when the workflow file is missing or you don't have permission to read it; or when a table name it needs collides in a way the import can't resolve on its own. If you see a server error instead, it carries a request ID — include it when you contact support.
...
### Alteryx Double Columns Keep Full Precision

An Alteryx **Double** column now converts to a 64-bit numeric column, matching the precision Alteryx itself calculates with. It previously converted to a 32-bit column — about 7 significant digits — so any value with more digits than that was silently rounded on read, and every calculation built on it inherited the error. A real case: 16,862,441.27 came back as 16,862,442.00.

Inside a converted macro, a Double column passed in as a macro input used to arrive as text; it now arrives as a number. If macro logic depended on the old text behavior — string concatenation, leading-zero formatting, a text pattern match — review it after reconverting, since a genuine number in hand can behave differently.

Workflows converted before this fix keep the column types they were converted with. Import them again to pick this up.
...
<Aside type="caution" title="Batch Macros That Used To Stall Now Complete">
Earlier conversions had each pass write its own output table, with the step gathering the results reading them all back by name — so a macro driven by as few as a few dozen records built one query spanning that many tables, and the database refused to plan it. The workflow stopped partway through, with nothing useful to point at as the cause.

Passes now accumulate onto a single table that the gathering step reads directly, and that table is emptied properly at the start of each run, so a run whose macro produces no passes returns an empty result instead of re-serving the previous run's rows.

If a batch macro workflow used to stall or fail partway through, re-import it to pick up the fix.
</Aside>

<Aside type="tip" title="Location Optimizer's Linear-Distance Case Converts Natively">
A **Location Optimizer** whose score is an unmodified linear distance-minimization — choose K sites minimizing total or weighted distance to demand — converts directly to the native [Location Optimizer](/reference/workflow-steps/optimization/location-optimizer/) step, a p-median MIP solved on PlaidCloud's HiGHS solver. The candidate and demand tables and the site count carry over; confirm the distance columns before relying on the result.
</Aside>

<Aside type="caution" title="Other Location Optimizer Macros Are Refused">
A **Location Optimizer** macro is only the scoring pass of a search. The search itself — generating candidate location sets, the number of locations, the generation limits and the seed — lives in the calling tool and in the Alteryx engine rather than in the workflow file. Outside the linear-distance case above — a gravity kernel, a bespoke scoring macro, a higher-is-better objective, or any transform of distance before scoring (a fixed facility cost, a Summarize, a join) — PlaidCloud has no equivalent for that optimisation loop. The macro call now arrives as a step flagged for review, naming the macro, the input its candidate sets arrive on, the output that ranks them, and whether a high or a low score wins.
