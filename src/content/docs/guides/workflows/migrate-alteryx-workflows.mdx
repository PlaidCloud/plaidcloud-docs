...
An Alteryx **Calgary database** (a `.cydb` file) is a proprietary indexed store. PlaidCloud has no way to read the file format, and there is no open specification for it. Rather than refuse every tool that touches one, conversion treats each `.cydb` path as standing for one ordinary PlaidCloud table, named `calgary_<database name>` under the project's table prefix. Once the database is an ordinary table, the tools built around it become ordinary operations: a saved query is a `WHERE` clause, cross-count buckets are conditional counts, and a loader's field list is a projection.
...
- A converted **Calgary Join** matches incoming records against the database through one of its indexes. It converts to a join against the table when the incoming field is a plain, non-spatial value — see [Calgary Join and Cross Count Append](#calgary-join-and-cross-count-append) below for when it does not.
...
A Calgary query is a tree of `And`, `Or`, and `Not` over the database's indexed fields, and the tree now converts as written: an `Or` anywhere in it, and a `Not` wrapped around more than one condition, both convert. This covers a Calgary Input's record read, its count-only read, Calgary Cross Count, and the buckets inside a Cross Count's custom fields.

A handful of query shapes still don't convert, each naming itself in the conversion report and on the step:

- A query using contains, starts-with, or a spatial lookup instead of an indexed comparison.
- A read limited by **Skip Records** or **Max Records** — the stand-in table carries no record order to skip or cap against.
- A Cross Count grouped by more than one custom field. The counts are genuinely one aggregate over the database, but Alteryx doesn't document what it names the resulting columns, and there is no non-spatial example in hand to check against.
...
- an incoming field typed as a spatial object refuses, naming the stand-in table and telling you to rebuild the match as a **Spatial Match** step against it;
- an incoming field whose type couldn't be determined refuses, naming that as the reason — it is never guessed;
- an ordinary, non-spatial incoming field converts a **Calgary Join** to a join against the stand-in table.

**Calgary Cross Count Append refuses on every incoming field, spatial or not.** It appends the *counts* of matching database records rather than the records themselves, and Alteryx doesn't document how many columns that produces or what they're named for anything beyond a single field — so there is no shape to build with confidence. Rebuild it as a **Spatial Match** or ordinary join against the stand-in table, followed by the aggregation you need.

A **Calgary Join** also still refuses, naming itself, when it is set to count matches only, when it matches on an index built over a range rather than a value, or when its unmatched-records output is wired to something downstream.
