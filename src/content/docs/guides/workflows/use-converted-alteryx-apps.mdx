...
The app's questions become App Runner widgets: text, numeric, date, check box,
radio button, drop-down, list box, tree, file browse, and folder browse. Each
widget writes to the workflow variable the converted step reads, so a run from
the App Runner drives the same logic as setting the variables by hand. A widget
