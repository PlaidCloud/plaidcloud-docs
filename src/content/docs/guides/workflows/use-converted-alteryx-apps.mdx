...
The form is laid out the way the app's author built it. Alteryx apps nest their
questions inside tabs and labelled boxes, and that grouping is carried through:
a start month and start year the author put together appear together, under
their own heading. Questions the author left ungrouped appear on their own.

Two kinds of control get more than a plain text box:

- A file or folder question has a **Browse** button that opens a document
  browser, so you pick the path instead of typing it from memory. Browse offers
  the Document accounts you can reach — the app's own account to begin with, and
  you can switch to another — then a path within the account you chose. A trail
  across the top walks back up, a folder in the list opens it, and **Select**
  shows what it will fill in before you press it. With nothing highlighted,
  Select takes the folder you are looking at, which is how you choose the
  account's top level.
- A numeric question honours the minimum, maximum and step the author set, so a
  question constrained to a small range cannot be answered outside it.
...
Answers are checked **before** the run starts. A choice question that was never
answered — one whose saved default names none of its choices, so nothing is
preselected — stops the run and is named, rather than being discovered part-way
through a run that may take the better part of an hour.

Once a run is under way the form shows what it is doing: a progress bar over the
steps completed, the name of the step running now, and the elapsed time. Run
stays disabled until the run finishes, so the same app cannot be started twice
over itself.
