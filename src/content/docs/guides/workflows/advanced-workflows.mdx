...
- **Undo** and **Redo** step backward and forward through layout changes (Cmd/Ctrl+Z and Cmd/Ctrl+Shift+Z).

### Annotate

Annotations are visual only — they document the diagram and never affect execution.

- **Add Note** — drop a sticky note anywhere on the canvas, then **Edit Text…** to write in it.
- **Add Highlight** — draw a translucent box around a group of related steps and give it a label with **Edit Label…**.
...
<Aside type="caution">The workflow must stay a DAG: a connector that would create a cycle is rejected ("that connector would create a cycle"). Remove a connection with **Remove Connector**.</Aside>
...
Several people can open the same Advanced workflow at once and work together live:

- **Presence** — a stack of collaborator avatars (their initials) shows who's viewing the workflow right now, much like Google Docs.
- **Live cursors** — you see each collaborator's cursor, labeled with their name and color, move around the canvas in real time.
- **Live layout and structure** — when someone rearranges the canvas, adds a step, or changes a connection, everyone sees the change as it happens.

No setup is required — collaboration is on whenever more than one person has the canvas open.
