...
An execution container can run its sealed sub-DAG **once per iteration** of a set of values — so a group that loads and transforms one region's data can process every region in turn without you copying the steps once per region. Right-click an execution container and choose **Container Settings…**, then the **Iterate Over** tab (the settings appear only on execution containers, since iteration drives a group that runs as a unit).
...
An execution container can also carry **conditions** that gate whether the whole container runs — the same variable, table, document, and time checks a step's conditions use, so the two work the same way. Right-click the container, choose **Container Settings…**, and open the **Conditions** tab: it's the [same Conditions UI a step uses](/guides/workflows/step-conditions/), unchanged.
