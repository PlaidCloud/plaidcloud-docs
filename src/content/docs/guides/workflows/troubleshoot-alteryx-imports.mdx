...
- The conversion report lists a missing file.
- A run fails with "Input file not found at &lt;path&gt;".
- Dynamic input resolves to no files.
- Validation row counts are lower than expected.

A workflow converts even when a file it reads is missing, so the report is the fastest way to see what's outstanding — it names the Document path to upload each missing file to, the original Alteryx path, any expected sheets, and the tools that read it.

Actions:

1. Upload each file the report lists to the Document path it names.
2. For most files, rerun the workflow once uploaded — no re-import needed.
3. For a missing flat ASCII layout, wildcard input, Dynamic Input template, or FirstRow rename source, import the workflow again instead of only rerunning it — those are read at conversion time.
4. Confirm dynamic file patterns still match after upload.
...
The file was written by the Alteryx AMP engine in a proprietary format that PlaidCloud cannot read. Rewrite it with Alteryx Designer while you still have it — re-save files individually, or batch-convert a whole folder to CSV with a downloadable workflow. See [Convert .yxdb Files for Import](/guides/workflows/convert-yxdb-files-for-import/).

## Unreadable .cydb (Calgary) Databases
...
The Alteryx Calgary database is a proprietary indexed store with no open reader, so PlaidCloud cannot load a `.cydb` directly. The Calgary tools themselves convert — each `.cydb` becomes an ordinary [stand-in table](/guides/workflows/migrate-alteryx-workflows/#calgary-databases) named `calgary_<database>` — but you supply that table's data yourself:
...
Specialized operations such as fuzzy matching, spatial processing, PDF extraction, OCR, NLP, and reporting can run through managed executors. (Machine-learning pipelines convert to the native [ML: Train Model](/reference/workflow-steps/machine-learning/ml-train/) and [ML: Score](/reference/workflow-steps/machine-learning/ml-score/) steps instead.)
