...
- Alteryx flat ASCII files: include the `.flat` layout file next to the data it describes. The column widths live only in that file, and conversion reads it to build the step, so a missing layout file still converts, but the conversion report names the file to upload — supply it and import the workflow again rather than just rerunning it.
...
If any workflow writes formatted output into a styled Excel template — the
format-preserving [Export to Excel](/reference/workflow-steps/export/export-to-excel/)
and [Export to Excel Sheets](/reference/workflow-steps/export/export-to-excel-sheets/)
steps — stage those template workbooks into the document account alongside the
inputs. Put them in a `04_Templates` folder next to the `03_Inputs` you staged
the source data in, so the converted step can read the template it fills.

<Aside type="caution">
  A converted format-preserving export reads its template from the document
  account **at run time**. If the template is missing when conversion runs, the
  step falls back safely to a plain export — but a converted format-preserving
  export whose staged template is absent at run time **fails loudly** rather than
  silently writing an unformatted file. Stage `04_Templates` before you run the
  converted workflow.
</Aside>
