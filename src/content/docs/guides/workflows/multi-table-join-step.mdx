...
PlaidCloud validates the join as you build it and again when the step runs. If something is wrong — an unsupported configuration, a cycle, a duplicate alias, an output column with neither a source nor an expression, or a column that no longer exists — the tab picks up a ⚠ badge with the count, the designer marks the offending table or join, and the save is rejected with the reason. Saving with unresolved problems switches you to the tab holding the first one; **Jump to issue** does the same on demand.
