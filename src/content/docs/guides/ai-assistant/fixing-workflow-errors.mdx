...
3. **You approve** — it waits for your go-ahead in a reply of its own. It cannot
   propose a change and treat the same message as your approval of it.
4. **Applies** — once you approve, it writes the corrected configuration to the
   step, through the same path the step editor uses. What gets saved is the
   configuration you were shown, and one approval saves it once.
5. **Re-run** — it tells you to re-run the step to confirm the fix.

<Aside>The assistant never changes your step without approval, and this is
enforced by PlaidCloud rather than left to the assistant's judgement. It shows
the proposed configuration first and applies it only after you explicitly
confirm that change in a later message — an ambiguous reply isn't taken as
approval. If it asks you to approve the same change a second time, nothing has
been saved yet.</Aside>
