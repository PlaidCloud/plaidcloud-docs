...
If you'd rather not compose a prompt from scratch, the wand button beside the input box offers a menu of common tasks, grouped by what they're for:

- **Explore** — explore a table, summarise this project, find where a table is used, explain a workflow
- **Build** — write a query
- **Learn** — look something up in the docs

1. Click the wand button beside the message box
2. Pick a category, then a task
3. Fill in what the task asks for — a table name, a workflow name, or what you want to know — and click `Start`

Most tasks write the message for you and put it in the message box. Read it, change anything you want, then press Enter to send it — nothing is sent until you send it.

**Find where a table is used** is the exception: it answers on the spot. PlaidCloud reads the answer out of the project's own records — which steps take the table as an input, and which write it — so the question and its answer both appear in the conversation straight away, with no AI model involved and nothing for you to send. Follow up on the answer as you would any other reply.

That answer also says what it searched: the steps in your project that name the table directly, and the projects yours shares data with. A step that reaches the table through a variable path is not found this way, and the answer says so every time — including when it finds nothing at all.

<Aside>A task that needs only the project itself, such as **Summarise this project**, skips the form and fills the message box straight away.</Aside>

<Aside type="note">Where a task asks for a table or a workflow, pick it from the list or type its name.</Aside>
