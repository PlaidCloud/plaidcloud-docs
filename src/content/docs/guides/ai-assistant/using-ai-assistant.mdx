...
When the step joins two tables, the summary also says what it joins on and what the join does with the rows: every row of the first table is kept whether or not it matches, and where the second table matches more than once only one of those rows is used, so a join never multiplies your rows. Name the columns you want in a join — asking for everything from both tables would write two columns under the same name, which a step cannot do.

The destination table has to exist already; ask the assistant to create one first if it doesn't. New steps are left unattached unless you name a workflow, and attaching to one is part of what you approve rather than something added afterwards.
