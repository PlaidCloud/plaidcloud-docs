...
The assistant can build a step that reads one table and writes the columns you want into another. Say what you want in plain language:
...
- "Create a step that copies Claims Cost into Claims Archive, renaming amount to paid_amount"

Between them these cover keeping, renaming, and dropping columns, filtering rows, and grouping with totals. Name your tables however you normally say them — the assistant matches on the name you use, and asks which you meant if two tables share one.

It does not yet join two tables, compute a column from an expression, add a constant column, or change a column's type — build those in the workflow editor. Ask the assistant for one and it will say so rather than building something else.

It always proposes before it builds, and **nothing is created until you reply**. The summary is the whole of what you are agreeing to, so it spells out every choice the step makes: the table it reads, each column it writes and what any renamed one came from, the row filter, whether the step groups and what it computes for each group, the table it writes into, the folder it files the step under, and whether it joins a workflow. If the proposal isn't what you meant, say what to change and it proposes again — and if anything it described has moved in the meantime, it asks you to approve the new version rather than building the old one.
