...
The assistant can build a step that reads one table — or joins two — and writes the columns you want into another table. Say what you want in plain language:
...
- "Create a step that joins Claim Count to LAE Pool on line and period"
- "Build a step that filters the claims table to 2025 and keeps only the line, region, and amount columns"

Between them these cover keeping, renaming, computing, and dropping columns, filtering rows, grouping with totals, and joining two tables on one or more columns. Name your tables however you normally say them — the assistant matches on the name you use, and asks which you meant if two tables share one.

It always proposes before it builds, and **nothing is created until you reply**. The summary is the whole of what you are agreeing to, so it spells out every choice the step makes: each column it writes and, where that column isn't a straight copy, what it was renamed from, what it computes, what constant it holds, or what type it is converted to; where each source's columns came from; any filter on the rows going in and any filter on the rows coming out; whether the step groups, and what it totals; the table it writes into; the folder it files the step under; and whether it joins a workflow. If the proposal isn't what you meant, say what to change and it proposes again — and if anything it described has moved in the meantime, it asks you to approve the new version rather than building the old one.

When the step joins two tables, the summary also says what it joins on and what the join does with the rows: every row of the first table is kept whether or not it matches, so a join never multiplies your rows. Where the second table matches more than once, only one value per column survives — the lowest — which means the matched values need not all come from the same row of that table. If that matters for your data, group or filter the second table down to one row per key first. Name the columns you want in a join, too — asking for everything from both tables would write two columns under the same name, which a step cannot do.
