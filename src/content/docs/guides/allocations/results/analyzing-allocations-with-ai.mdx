...
Because a what-if rests on today's shares, it also warns you when the data behind those shares has not been reloaded recently — naming the pool or the driver, how many days ago it was last loaded, and the window your administrator set. The estimate is still given; the warning tells you what it is resting on. This needs stale-data flagging to be switched on for your workspace, described under [Tracing Allocations](/integrations/ai-coding-agents/tracing-allocations/#how-far-to-trust-an-answer).
</Aside>
