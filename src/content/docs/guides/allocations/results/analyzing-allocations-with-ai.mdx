...
Where the answer is that the share moved, it names the **driver** that share is split on, and this slice's share of that driver either side — "the shift in this slice's share of the pool (split on the revenue driver — this slice's share of that driver went 23.2% to 29.3%)". That is the part you can act on: a share is a result, a driver is an input someone owns and can check. Note which share those two figures are: they are shares of the **driver**, not of the pool the same sentence mentions. The two differ wherever part of the pool passes through without being split, which is why the sentence spells it out. Two cases give the driver and no figures. Where the value being traced is negative — a loss-making line, say — its share of the driver falls outside 0–100% and a percentage pair would mislead rather than inform. And a step with no driver at all has none to name, so the sentence reads as it always did.
