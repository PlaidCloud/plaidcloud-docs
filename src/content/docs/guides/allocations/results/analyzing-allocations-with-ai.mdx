...
Where the answer is that the share moved, it names the **driver** that share is split on and the slice's share either side — "the shift in this slice's share of the pool (split on the headcount driver, 26.5% to 18.2%)". That is the part you can act on: a share is a result, a driver is an input someone owns and can check. Two cases give the driver and no figures. Where the value being traced is negative — a loss-making line, say — its share of the pool is negative too, and stating it would mislead rather than inform. And a step with no driver at all has none to name, so the sentence reads as it always did.
