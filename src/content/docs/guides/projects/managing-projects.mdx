...
### Copying Only Part of a Project

The duplicate dialog lists the project's contents grouped by kind — workflows, steps, functions, tables, data editors, and dimensions. Everything is ticked to start with, so leaving the tree alone copies the whole project.

To copy only part of it, untick whatever you don't want, or expand a kind and tick individual items. As with restoring an archive, anything your selection depends on comes with it — a workflow brings its steps and their tables and functions, a dimension brings the tables its members are built from, and a data editor brings its source and target tables. If you start the copy with nothing ticked, PlaidCloud asks you to pick something rather than create an empty project.
