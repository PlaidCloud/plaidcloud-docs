...
## Setting a Default Document Account

On the project home, next to **Connection Environment**, pick a **Default Document Account** for the project — or **None** to clear it.

Inside the project, every file selector lists **Project Default** first, standing in for whichever account the project currently has set. It drops off the list once you leave the project, once the project has no default set, once that account has been deleted, or once you lack access to it.

A new file step preselects Project Default when the project has one. Existing steps are never rewritten — each keeps the account it was saved with until you change it yourself, since a document account is a per-step choice in a way the connection environment is not.

A step left on Project Default resolves to whichever account is the project's default when it runs, so changing the default mid-run affects only the nodes that have not started yet — the same as changing the connection environment. If a step runs on Project Default and the project has no default set, the run fails and names the project, rather than falling back to another account.
...
A step set to **Project Default** carries that setting across, not an account — once copied, it resolves to whatever default the target project has, not the one it had in the source.
