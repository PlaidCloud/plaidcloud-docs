...
A step left on Project Default resolves to whichever account is the project's default when it runs, so changing the default mid-run affects only the nodes that have not started yet — the same as changing the connection environment. If a step runs on Project Default and the project has no default set, the run fails with a clear error rather than falling back to another account.
