...
* **May Reference This Table** — steps that reach a table through a variable whose value is not known right now. This section only appears when there are such steps.
...
Some variables have no value to resolve. A Workflow Loop sets its variables from data as it runs, and a Set Variable step assigns during a run, so whatever is stored between runs is left over from the last one. Rather than resolve those to a table that may be the wrong one, PlaidCloud lists the steps by name under **May Reference This Table**, with the variable it could not resolve, so you can open them and check.
