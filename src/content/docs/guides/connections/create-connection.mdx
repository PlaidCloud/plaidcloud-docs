...
Password, token, and other credential fields are write-only: they show blank when you reopen a saved connection, and leaving them blank on save keeps the stored secret. Retype a value only when you want to change it. This covers a REST connection's API key, OAuth client secret, and OAuth refresh token. A handful of multi-line credentials — a SQL or Git connection's SSL client key and SSH private key, a Postman API key, a Slack or Teams incoming-webhook URL — are write-only the same way but stay visible while you're actively typing them, since a multi-line field can't be masked. Every other field shows its real saved value everywhere the connection is shown, in the editor and over the API, including fields that look sensitive by name (a database's read-only flag, an account identifier, a schema, a host name) — only an actual credential is ever masked or hidden.

Over the REST API and MCP, a credential field follows the same rule as the editor: send an empty string and PlaidCloud keeps the stored value; send `null` to clear it. Every non-credential field still clears when you send it blank.
...
<Aside type="note">**Test Connection** appears on a connection kind once its editor offers it — currently the database types above plus [TM1](/guides/connections/tm1/), [Oracle Essbase](/guides/connections/essbase/), [HFM/FCCS](/guides/connections/hfm-fccs/), Salesforce, and Anaplan. Other REST connections, like NetSuite, don't have it yet — smoke-test those by sending a small request from a workflow step (for NetSuite, see [Connect to NetSuite](/guides/connections/netsuite/#smoke-test-the-connection)).</Aside>
