...
<Aside type="caution">
This connector is built to D365 F&O's documented OData and DMF endpoints and has not yet been validated against a live D365 F&O environment. Run every import against a sandbox environment first, and work through **Validate Only** below on every post batch before posting live.
</Aside>
...
<Aside type="caution">
- **No live tenant validation.** Built to D365 F&O's documented OData and DMF endpoints; not yet exercised against a live D365 F&O environment. Validate field mappings and posting behavior in a sandbox environment before relying on this step in production.
