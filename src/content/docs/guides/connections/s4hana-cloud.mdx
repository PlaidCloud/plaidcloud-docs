...
<Aside type="caution">
This connector is built to SAP S/4HANA Cloud's documented OData and SOAP APIs and has not yet been validated against a live S/4HANA Cloud tenant. Run every import against a sandbox tenant first, and work through **Validate Only** below on every post batch before posting live.
</Aside>
...
<Aside type="caution">
- **No live tenant validation.** Built to S/4HANA Cloud's documented OData and SOAP APIs; not yet exercised against a live S/4HANA Cloud tenant. Validate field mappings and posting behavior in a sandbox tenant before relying on this step in production.
