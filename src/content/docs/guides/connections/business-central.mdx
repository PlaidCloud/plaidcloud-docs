...
<Aside type="caution">
Run every import against a sandbox environment first, and work through **Preview and Validate** below on every post batch before posting live.
</Aside>
...
<Aside type="note">
**The company comes from the connection, not from the step.** Business Central scopes every read to one company, and the connection carries that company along with the environment — so the import step has no company or environment field. To pull the same entity from a second company, create a second Business Central connection pointing at it.
</Aside>
