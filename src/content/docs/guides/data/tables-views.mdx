...
<Aside>Using views can help speed up workflows since no data movement is necessary at workflow run time.</Aside>

<Aside>Since views contain no data, you will notice that they cannot be used as a target for imports. A table must be used in that case.</Aside>
