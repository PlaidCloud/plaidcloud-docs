...
The toolbar's **Row Access** button opens [Row Access](/administration/access/managing-security-groups-and-assignments/#managing-row-access) for the table you're exploring — a badge on the button shows when the table already has a restriction. A column's facet menu also has a **Restrict Rows by This Column** action that opens the same screen pointed at that column.
...
[Vector](/guides/data/vector-data-type/) columns are the one exception: they are left out of profiling and no list of values is built for them. A minimum, a mean, or a list of top values says nothing about an embedding, and the value list would be raw arrays of hundreds of numbers each.
