...
<Aside>An agent with no group set, or the only agent in its group, serves every environment. Nothing changes for a workspace that runs a single agent, and setting these two fields on a lone agent is safe.</Aside>

<Aside type="caution">Once a group has two or more agents, the match becomes exact: a project running in an environment no agent in the group serves fails with a message naming the group and the environment, rather than falling back to whichever agent the step named. That is deliberate — the alternative is production work quietly running on a test machine. Add an agent for each environment your projects run in.</Aside>
