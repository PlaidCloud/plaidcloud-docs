---
title: GENERATE_SERIES (Lakehouse v1)
description: GENERATE_SERIES — generates a dataset starting from a specified point, ending at another.
---
...
<Aside>When dealing with functions like GENERATE_SERIES and RANGE, a key distinction lies in their boundary traits. GENERATE_SERIES is bound by both the left and right sides, while RANGE is bound on the left side only. For example, utilizing RANGE(1, 11) is equivalent to GENERATE_SERIES(1, 10).</Aside>
