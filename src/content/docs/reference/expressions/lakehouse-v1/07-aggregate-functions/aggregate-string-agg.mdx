---
title: STRING_AGG (Lakehouse v1)
description: STRING_AGG — concatenate non-NULL column values into a single string, separated by a delimiter.
---
...
<Aside>If `<expr>` is not a String expression, should use `::VARCHAR` to convert. For example: ```sql SELECT string_agg(number::VARCHAR, '|') AS s FROM numbers(5); ┌───────────┐ │ s │ ├───────────┤ │ 0│1│2│3│4 │ └───────────┘ ```</Aside>
