...
| ~100,000-row SuiteQL ceiling | NetSuite caps a single query at roughly 100,000 rows without SuiteAnalytics Connect. A hand-written query that would cross it fails loud with a message asking for a narrower date range or other filters, rather than silently truncating. The canned [NetSuite: Import Balances / GL Detail](/reference/workflow-steps/netsuite/) steps window a date range for you automatically to stay under it. |
| Multi-book accounting | On a Multi-Book Accounting account, `TransactionAccountingLine` carries one row per book per transaction line — an unfiltered `SUM` across books double- (or N-times-) counts. The canned import steps default to the primary book; a hand-written query should filter `accountingbook = 1` (or your chosen book) explicitly unless a cross-book total is what you actually want. |
