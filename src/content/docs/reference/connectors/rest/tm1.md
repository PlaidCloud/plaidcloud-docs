...
| Default Database | No | A convenience prefill for a TM1: Query Cube step's Database field. Each step can override it. |
...
- **Read cube data by saved view or MDX.** The [TM1: Query Cube](/reference/workflow-steps/ibm-planning-analytics/tm1-query/) step runs a named cube view or a raw MDX query, applies an optional point-of-view slice, and lands the result as a table.
- **Load a TM1 hierarchy as a PlaidCloud dimension.** The [TM1: Read Dimension](/reference/workflow-steps/ibm-planning-analytics/tm1-dimension-read/) step reads a dimension's elements, parent/child edges, and attributes, and builds a PlaidCloud dimension from them.
...
TM1: Query Cube (Agent) has no live discovery — an agent is only reachable during a run, not while the step's form is open — so cube, view, and MDX are typed by hand and validated when the step runs. TM1: Read Dimension is cloud-direct only; it has no on-premises path yet.
...
| On-prem reach is query-only | The on-premises PlaidLink agent path covers cube reads (TM1: Query Cube (Agent)). TM1: Read Dimension has no on-premises path yet — it needs a network path PlaidCloud can reach directly. |
| Dimension read is single-hierarchy, single-parent | A TM1 hierarchy with an element under more than one parent cannot be loaded as a PlaidCloud dimension, and a consolidation weight's magnitude (0.5, 2.0) is not representable — only its sign. See [TM1: Read Dimension](/reference/workflow-steps/ibm-planning-analytics/tm1-dimension-read/#what-this-step-cannot-represent). |
...
- [TM1: Query Cube](/reference/workflow-steps/ibm-planning-analytics/tm1-query/) — cube reads, cloud-direct.
- [TM1: Query Cube (Agent)](/reference/workflow-steps/ibm-planning-analytics/tm1-query-remote/) — cube reads through an on-premises agent.
- [TM1: Read Dimension](/reference/workflow-steps/ibm-planning-analytics/tm1-dimension-read/) — load a TM1 hierarchy as a PlaidCloud dimension.
