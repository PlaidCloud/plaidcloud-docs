...
If the source table has no rows when the step runs, the export finishes with a warning that the source was empty rather than a silent success — so an unexpectedly empty target is easy to spot. Most often this means the workflow had not yet built that table; make sure the source is populated (and, for scheduled runs, ordered to run first) before the export.
</Aside>
