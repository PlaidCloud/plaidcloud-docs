...
A `literal_column` ending in a comment — `--`, or `#` on a MySQL-compatible warehouse — is refused for the same reason: the comment runs to the end of the statement, so it erases whatever PlaidCloud adds after your expression rather than only what you wrote. Put the comment somewhere it cannot reach the rest of the query, or leave it out.
