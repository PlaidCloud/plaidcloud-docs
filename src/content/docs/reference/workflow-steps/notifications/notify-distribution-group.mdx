---
title: Notify Distribution Group
description: Send notifications to a distribution group from a PlaidCloud workflow step to alert multiple recipients about workflow events.
sidebar:
  order: 1
---

import { Aside } from '@astrojs/starlight/components';

## Description



Sends an email notification to a PlaidCloud distribution group — a named list of recipients managed in the workspace settings. Useful for status updates, completion notifications, or error alerts that need to reach a defined audience.

Messages are sent from PlaidCloud's notification service. Distribution groups are managed centrally so workflow authors don't embed email addresses in workflow definitions.


## Select PlaidCloud Distribution List


Select a single distribution list from the drop down menu. Distribution lists can be created using Tools. For details on creating a distribution list, see here: [PlaidCloud Tools – Distro](/administration/access/member-management/#managing-distribution-distro-lists).



## Message


Specify **Subject** and **Body** as desired.



Please note that both Project Variables and Workflow Variables are available for use with this transform, in both the subject line and the message body.



Additionally, standard HTML code is permitted in the body to further customize the look of the email messages.


## Examples


In this example, all of the system variables are used. Additionally, there is a small bit of HTML used to format the first line of the body. Executing this transform will send the following email to all members specified in the distribution group:


* FROM: [info@tartansolutions.com](mailto:info%40tartansolutions.com) (remember that all messages come from this address)
* Subject: DEMO Analyze Demo Running


<Aside>Individual recipients of the email message will **not** be able to see the names of other members on the distribution list.</Aside>
