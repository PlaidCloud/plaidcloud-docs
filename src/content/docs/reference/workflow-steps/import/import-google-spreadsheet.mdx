...
Imports specific worksheets from a Google Sheets spreadsheet into PlaidCloud project tables. Useful for ad-hoc data that lives in spreadsheets — driver tables maintained by business users, lookup mappings, manual overrides.

Authenticates via a Google connection; multiple worksheets from one spreadsheet can be imported in one step.
