...
Each new column is named after the value it came from in the category column. By default that value is used **exactly as it appears** — a category value of `Freight Revenue` produces a column named `Freight Revenue`, spaces and all.

Turn on **Sanitize new column names** to replace spaces and punctuation in those names with underscores — for example, `Freight Revenue` becomes `Freight_Revenue`. This matches how Alteryx names its CrossTab output fields, so workflows imported from an Alteryx CrossTab tool enable it automatically to keep their later steps lined up. Leave it off for the readable, as-typed names most models expect.
