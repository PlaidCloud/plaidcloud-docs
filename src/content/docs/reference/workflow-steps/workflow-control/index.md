---
title: Workflow Control Steps
description: Steps that control workflow execution itself — variables, loops, error handling, and sub-workflow invocation.
---

Steps that control workflow execution itself — variables, loops, error handling, and sub-workflow invocation.

## Steps

- [Clear Workflow Log](/reference/workflow-steps/workflow-control/clear-workflow-log/)
- [Copy Workflow](/reference/workflow-steps/workflow-control/copy-workflow/)
- [Create Workflow](/reference/workflow-steps/workflow-control/create-workflow/)
- [Delete Workflow](/reference/workflow-steps/workflow-control/delete-workflow/)
- [Macro Concurrent](/reference/workflow-steps/workflow-control/macro-concurrent/)
- [Macro Run](/reference/workflow-steps/workflow-control/run-macro/)
- [Raise Workflow Error](/reference/workflow-steps/workflow-control/raise-workflow-error/)
- [Rename Workflow](/reference/workflow-steps/workflow-control/rename-workflow/)
- [Row Count Assert](/reference/workflow-steps/workflow-control/row-count-assert/)
- [Run Workflow](/reference/workflow-steps/workflow-control/run-workflow/)
- [Set Project Variable](/reference/workflow-steps/workflow-control/set-project-variable/)
- [Set Workflow Variable](/reference/workflow-steps/workflow-control/set-workflow-variable/)
- [Stop Workflow](/reference/workflow-steps/workflow-control/stop-workflow/)
- [Workflow Loop](/reference/workflow-steps/workflow-control/workflow-loop/)
