...
Allocates values from a source table to targets based on an assignment dimension and a driver data table. This is the most flexible allocation step — use it for driver-based spreading where each source row's destination is determined by a hierarchy and driver values determine *how much* goes where.

Common patterns: spread cost center totals to product lines using activity hours; distribute IT overhead across business units based on headcount; allocate shared service costs using a custom weighting.
...
<Aside>When allocating ledger values _Account_ or _Cost Center_ dimensions are normally used.</Aside>
...
- **Property Type** - This property informs the allocation step which table ***Values To Allocate Table*** or the ***Driver Data Table*** this dimension is related too. 
  ...
  - **Context** - When the ***Values To Allocate Table*** and the ***Driver Data Table*** contain the same dimension then context can be used to specify how the dimensions should relate to one another. Context is often used when both the ***Values To Allocate Table*** and the ***Driver Data Table*** contain _Profit / Cost Centers_ or _Geography_.
    - **Left blank** - A node with no context of its own takes the context of the nearest ancestor that sets one, so a blank cell means _inherit_, not _no context_. Only when no ancestor sets one either does the ***Driver Data Table*** fall back to being filtered by the target dimension's own setting.
    - **Current** - Acts as a passthrough and will filter the ***Driver Data Table*** based on the settings of the target dimension. An example would be if the ***Context*** is based on the _Profit Center_ dimension and the _Profit Center_ target dimension is set to ***ALL*** then the driver data would filter on all _Profit Centers_. Because it is a stored value it also stops the inheritance described above, so ***Current*** is how a node says "no context here" when an ancestor sets one — clearing the cell instead would hand the node its ancestor's context.
    ...
    - **Unassigned** - In place of the driver rows the target dimension would select, only those whose _Profit Center_ has no value at all or reads `UNASSIGNED` (in any capitalisation) are used, and the _Profit Center_ does not split the result. Use it for a rule that spreads costs no _Profit Center_ owns. It governs only the rule that sets it — another rule leaving the same column blank is unaffected.
    <Aside>When ***Context*** is set to ***All***, ***Parent*** or ***Unassigned*** it will override the setting on the target dimension. ***Current*** does not override it — it defers to it, which is also what a blank cell does once no ancestor has set a context.</Aside>
...
A record that no rule matches has empty rule columns in the **Mapping** table, and is counted on the **Summary** row that names no rule. A value a rule leaves blank is empty as well. Empty means a null, never an empty string, and the three tables have the same columns and types, whether or not **Apply Rules using SQL, not Pandas** is ticked.
...
<Aside type="danger">Setting either or both of the ***Thread Count*** or ***Chunk Size*** to high will slow the allocation processing. Slowly incrementing these values up and observing performance is the ideal way of tuning the allocation step.</Aside>
