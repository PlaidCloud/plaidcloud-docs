...
PlaidCloud publishes new releases continuously and delivers them to your workspaces automatically. You don't request or install an update — when a release reaches a workspace's release channel, PlaidCloud upgrades the workspace during its maintenance window. The release channel decides how quickly a release arrives; the maintenance window decides what time of day the upgrade lands.

### How a Release Reaches a Workspace

1. PlaidCloud publishes a release to a channel. Rapid receives it first, then Regular, then Stable.
2. Everyone in the organization receives an advance-notice email at least seven days before the upgrade, naming the release and the date it applies.
3. On the workspace's first maintenance window after that notice, PlaidCloud upgrades the workspace to the release. The default maintenance window is Sunday at 02:00 UTC — set the day and time under [Maintenance](#maintenance).
4. The workspace's **Version** updates to the new release. No action is required from you or from workspace users.

Because a workspace only upgrades inside its own maintenance window, updates land at a predictable, low-traffic time, and the seven-day notice gives every organization time to plan around the change.

### Opt Out by Pinning a Workspace

To hold a workspace on its current version, set its **Release** channel to **No Release (pinned)**. Automatic upgrades skip a pinned workspace, and it stays on its selected version until an administrator picks a new version or moves it back onto a channel. Pin a workspace when it must stay fixed — for example, through a validation cycle or a regulated close.

Pinning stops future upgrades only; it doesn't reverse an upgrade that already applied. To move a pinned workspace forward again, choose a specific **Version** or select the Rapid, Regular, or Stable channel, and the workspace rejoins the automatic schedule.
