...
The workspace table includes name, unique ID, version, release channel, status, paused state, created date, provisioning state, last error, cluster, maintenance day, and maintenance time. The table supports search, refresh, row actions, and bulk actions.
...
Creating a workspace takes several minutes, and until it finishes the workspace exists but cannot be opened. The **Created**, **Status**, **Provisioning**, and **Last Error** columns tell you where it got to, and the **Provisioning** tab on the workspace details dialog shows the same information for one workspace along with the service URLs currently published for it.
...
| Last Error | Empty unless provisioning failed. When it failed, this names the phase it failed on. |

A workspace that stays in the same phase for more than about half an hour has stalled — contact PlaidCloud support with the workspace ID and the phase shown.
