...
<Aside type="caution">
**An attribute name has to match its column name exactly** — case and whitespace included. `Region` and `region` are different names, and so are `region` and `region ` with a trailing space. Declaring an attribute whose name doesn't match any column on the bound table verbatim now warns you and names the closest column it found, instead of saving a governance rule that binds to nothing and silently leaves the table ungoverned.
</Aside>
...
<Aside type="caution">
**Publish a governed table before you declare row access on it — that's still the normal order.** Row Access restricts a table's *published reporting view*, so the table has to be published for reporting first — the [Apply to dashboards](#applying-row-access-to-dashboards) step binds and filters that view. Once a column is declared as governing, PlaidCloud declines to publish or republish that table, because an unfiltered reporting view would let anyone with reporting access read every row. A table already published stays published; only new publishes are declined. One consequence: while a table stays governed, a change to its shape cannot rebuild its reporting view — un-govern the column, republish, then declare it again if you need to. See [Publishing Tables](/guides/data/publish/).
</Aside>

<Aside type="caution">
**Declaring governance on a table that's already published enforces immediately, or refuses.** If you declare a governing column (with a deny-by-default value list) against a table that's already published, PlaidCloud enforces row-level security on that published view right away rather than leaving it open until you next apply. If enforcement can't be established at that moment — for example, this workspace's very first Row Access apply hasn't been [approved](#approving-the-first-apply) yet — the declare itself is refused, so a published view is never left readable without the governance you just asked for. A column left at its all-default, no-restriction setting has nothing to enforce and isn't affected, and the ordinary declare-then-publish flow above is unchanged.
</Aside>
...
- A governed table has to have been published to the project's report schema first, and every governed column has to still exist in what was published. Where one doesn't, the apply stops and names the column and the attribute bound to it, so you know which declared attribute to check rather than just which table failed.
...
Declaring or un-declaring a governed attribute, editing a group's grants, and applying (reconciling) a project all write an entry to that project's [Project Log](/get-started/tutorials/collaborate-versions-and-sharing/#read-the-project-log) — who made the change and when. Open **Analyze → Projects → Log** on the project to see it. Nothing about Row Access has to be turned on to get this trail; it's recorded the same way every other tracked change to a project is.
