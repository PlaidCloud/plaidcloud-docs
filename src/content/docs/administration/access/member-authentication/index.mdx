...
<Aside>Members may have access to the Identity tab for security purposes or in order to manage members for the workspace. Details on managing security and authentication for new members or members without access can be found on the main “Identity” page.</Aside>
...
<Aside>Only strong passwords are accepted, and the new password must be different from the current one.</Aside>


## Signing Out


To sign out, select the icon (gravatar) in the upper right and click “Sign Out” in the dropdown menu.

Signing out ends both your PlaidCloud session and your Single Sign-On session with your identity provider. Because your Single Sign-On session is ended too, you cannot be silently signed back in — so no one returning to a shared or still-open browser is signed straight back into your account, and returning to PlaidCloud means signing in again.

<Aside>Because signing out ends your Single Sign-On session, you will be asked to sign in again the next time you return to PlaidCloud.</Aside>
...
<Aside>Keys from the PlaidCloud YubiKey pool (YubiKeys specifically issued by PlaidCloud) count towards the five key limit.</Aside>
...
<Aside>If you do not have a backup code or a backup key registered a much more stringent process is followed that may require several days to validate the authenticity of the access request and maintain PlaidCLoud security.</Aside>
...
<Aside>This is not an automated process, so regaining access may require some time.</Aside>
