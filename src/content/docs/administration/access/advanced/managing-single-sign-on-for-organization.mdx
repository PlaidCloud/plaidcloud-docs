...
Each Organization can have a custom url ([https://plaidcloud.com/sso](https://plaidcloud.com)/`<custom_name_here>`) for members to access the single sign-on page you specified in the configuration.

<Aside>Single Sign-On uses SAML 2.0 protocols and is set up through the user interface.</Aside>
...
<Aside>If a member is marked as an administrator within a workspace, they will continue to have full access to that workspace regardless of the specific role they may be assigned through this automated process.</Aside>
