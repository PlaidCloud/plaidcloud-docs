...
- **Table Inner, Outer, Anti, and Cross Join — Output Columns fixes.** The Output Columns tab now lists only the columns you mapped from the two sources instead of every column, and it drops stale output and join-key rows when you remove a mapped source column. The column-mapper toolbar and multi-row selection are back, and **Summarize is now a toggle that's off by default** — a join no longer forces its output to be grouped. Turn Summarize on to aggregate; new columns default to an aggregation based on their data type, which you can override per column. See [Table Inner Join](/reference/workflow-steps/tables/table-inner-join/).
- **Multi-Table Join output columns follow the canvas.** Renaming a source in the join-graph designer now updates the output columns that referenced it, and removing a source drops its output columns — instead of leaving stale rows pointing at a table that's no longer there. See [Multi-Table Join Step](/guides/workflows/multi-table-join-step/).
