...
- **Table Inner, Outer, Anti, and Cross Join — Output Columns fixes.** The Output Columns tab now lists only the columns you mapped from the two sources instead of every column, the column-mapper toolbar and multi-row selection are back, and **Summarize is now a toggle that's off by default** — a join no longer forces its output to be grouped. Turn Summarize on to aggregate, and the per-column aggregation follows each column's data type. See [Table Inner Join](/reference/workflow-steps/tables/table-inner-join/).
