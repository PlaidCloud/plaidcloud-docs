...
- **AI allocation cost-tracing now tells you how confident it is — and flags when a confident-sounding number would mislead.** Each explanation carries a confidence signal, and warnings ride in the plain-language answer itself: a year-over-year comparison over a still-incomplete period or with no prior year to compare against, a whole-group total that hides a member-level reshuffle, a slice whose factors move together so the headline percentages aren't reliable, or a what-if whose per-target impacts can't simply be summed. See [Analyze Allocations With an AI Agent](/guides/allocations/results/analyzing-allocations-with-ai/).
