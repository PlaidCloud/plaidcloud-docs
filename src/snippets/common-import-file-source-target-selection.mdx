...
<Aside type="caution">When choosing a directory to import, ensure all files have the same format</Aside>
...
Inside a project, the list also offers **Project Default** at the top — a stand-in for whatever account the project currently has set as its default. A new step preselects Project Default when the project has one; an existing step keeps the account it was already saved with until you change it. A step left on Project Default runs against whichever account is the project's default at the time — if none is set, the run fails and names the project rather than falling back to another account.
...
<Aside>Using a common naming convention for files of similar type and purpose makes it easier to use a single import step</Aside>
